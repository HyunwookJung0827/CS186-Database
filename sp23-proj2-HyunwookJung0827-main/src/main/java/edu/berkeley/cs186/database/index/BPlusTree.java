...
//        LeafNode currLeaf = this.root.getLeftmostLeaf();
//        ArrayList<iterator> iterList = new ArrayList<iterator>();
//        while(!currLeaf.equals(Optional.empty())){
//            // Traverse through all (keys, RecordId)
//            iterList.add(currLeaf.scanAll());
//
//            // Go to the next LeafNode
//            currLeaf = currLeaf.rightsibling;
//        }
//
//        // After while loop, the array is completed.
//        // Make a BPlusTreeIterator out of the completed array recordList
//        BPlusTreeIterator amiwfealrughlaieurgh = new BPlusTreeIterator(iterList);
//
//        //return Collections.emptyIterator();
//        return amiwfealrughlaieurgh;
        return new BPlusTreeIterator(null, root.getLeftmostLeaf());
    }
    ...
    public Iterator<RecordId> scanGreaterEqual(DataBox key) {
        ...
        // TODO(proj2): Return a BPlusTreeIterator.
//        LeafNode currLeaf = this.root.get(key);
//        ArrayList<RecordId> recordList = new ArrayList<RecordId>();
//        // 1. Find the theKey inside this node such that theKey >= key for the first time
...
//        while(!currLeaf.equals(Optional.empty())){
//            // Traverse through all (keys, RecordId)
//            for (int i = 0; i < currLeaf.getRids().size(); i++) {
//                if (key.compareTo()){
//                recordList.add(currLeaf.getRids().get(i));
//                }
//            }
//
//            // Go to the next LeafNode
...
//        // After while loop, the array is completed.
//        // Make a BPlusTreeIterator out of the completed array recordList
//        Iterator<RecordId> it = recordList.iterator();
//        BPlusTreeIterator amiwfealrughlaieurgh = new BPlusTreeIterator(it);
//
//        //return Collections.emptyIterator();
//        return amiwfealrughlaieurgh;
//        return Collections.emptyIterator();
        return new BPlusTreeIterator(root.get(key).scanGreaterEqual(key), root.get(key));
    }
    ...
    public void put(DataBox key, RecordId rid) {
        ...
        Optional<Pair<DataBox, Long>> returned_key = this.root.put(key, rid);
        // Case 1: put returns Optional.empty
        if (returned_key.equals(Optional.empty())) {
            return;
        }
        // Case 2: put returns a value that forces a new root to be created
        ArrayList<DataBox> newKey = new ArrayList<>();
        ArrayList<Long> newChildren = new ArrayList<>();
        newKey.add(returned_key.get().getFirst());
        newChildren.add(this.root.getPage().getPageNum());
        newChildren.add(returned_key.get().getSecond());
        InnerNode newRoot = new InnerNode(this.metadata, this.bufferManager, newKey, newChildren, this.lockContext);
        updateRoot(newRoot);
    }
    ...
    public void bulkLoad(Iterator<Pair<DataBox, RecordId>> data, float fillFactor) {
        ...
        // the tree's root if the old root splits.
        while (data.hasNext()){
            Optional<Pair<DataBox, Long>> returned_key = this.root.bulkLoad(data, fillFactor);
            // Case 1: put returns Optional.empty
            if (returned_key.equals(Optional.empty())) {
                continue;
            }
...
//        public BPlusTreeIterator(Iterator<RecordId> iter, LeafNode leaf) {
        public BPlusTreeIterator(Iterator<RecordId> iter, LeafNode leaf) {
            this.leaf = leaf;
            if (iter == null) {
        ...
        public boolean hasNext() {
            // TODO(proj2): implement
            // If this.iter is empty now, update this.leaf
            if (!this.iter.hasNext()) {
                // If update failed, return false
                // If update succeeded, we found an element and return true
                return updateLeaf();
            }
        ...
        // helper function updateLeaf() { go to the next leaf}
        public boolean updateLeaf() {
            // Go to the rightsibling
            while (!this.iter.hasNext()) {
                // loop until this.iter has something or there is no more rightsibling available from this.leaf
                if (this.leaf.getRightSibling().isPresent()) {
                    this.leaf = this.leaf.getRightSibling().get();
                    this.iter = this.leaf.scanAll();
                }
                else {
                    return false; // False: Update failed (All leaves ran out)
                }
            }
            return true; // True: Update success.
        }
