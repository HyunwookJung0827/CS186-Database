...
class LeafNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        if (getKeys().contains(key)) {
            throw new BPlusTreeException("You cannot insert a duplicate key.");
        }
...
//            if (key.compareTo(getKeys().get(i)) == -1) {
//                // then we put there
//                getKeys().add(i, key);
//                getRids().add(i, rid);
//            }
//        }
        ...
        if (getKeys().size() > 2*d) {
            ...
            // This leaf become Left Node, we create new Right Node

            LeafNode R = new LeafNode(this.metadata, this.bufferManager, this.getKeys().subList(d, this.getKeys().size()),
                    this.getRids().subList(d, this.getKeys().size()), this.rightSibling,
                    this.treeContext);
            for (int j = 0; j <= d; j++) {
                this.getKeys().remove(d);
                this.getRids().remove(d);
            }
            this.rightSibling = Optional.of(R.getPage().getPageNum());
            Pair ret = new Pair(R.getKeys().get(0), R.getPage().getPageNum());
            sync();
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
            float fillFactor) {
        // TODO(proj2): implement
        // We start from an empty leaf
        // Go get the metadata
        int d = this.metadata.getOrder();
        int i = this.getKeys().size();
        // How many pairs do we add here? It's 2*d*fillFactor + 1 then split and return.
        for (; i < 2*d*fillFactor; i++) {
            if (data.hasNext()) {
                Pair<DataBox, RecordId> nextPair = data.next();
                this.getKeys().add(nextPair.getFirst());
                this.getRids().add(nextPair.getSecond());
            } else {
                sync();
                return Optional.empty();
            }
        ...
        // Case if we still have data, let's make a new leaf node
        if (data.hasNext()) {
            Pair<DataBox, RecordId> nextPair = data.next();
            ArrayList<DataBox> r_keys = new ArrayList<>();
            ArrayList<RecordId> r_records = new ArrayList<>();
            r_keys.add(nextPair.getFirst());
            r_records.add(nextPair.getSecond());
            LeafNode R = new LeafNode(this.metadata, this.bufferManager, r_keys,
                    r_records, this.rightSibling,
                    this.treeContext);
            this.rightSibling = Optional.of(R.getPage().getPageNum());
            Pair ret = new Pair(R.getKeys().get(0), R.getPage().getPageNum());
            sync();
    ...
    public static LeafNode fromBytes(BPlusTreeMetadata metadata, BufferManager bufferManager,
        ...
        // START: rightSibling = -1 fix (page allocation)
        Long rightSiblingPage = buf.getLong();
        Optional<Long> rightSibling = Optional.of(rightSiblingPage);
        if (rightSiblingPage == -1) {
            rightSibling = Optional.empty();
        }
        // END: rightSibling = -1 fix (page allocation)
