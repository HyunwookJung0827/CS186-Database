...
class InnerNode extends BPlusNode {
    ...
    public LeafNode get(DataBox key) {
        ...
        int child_index = numLessThanEqual(key, getKeys());
        return getChild(child_index).get(key);
    }
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        int child_index = this.numLessThanEqual(key, this.getKeys());
        BPlusNode next_node = this.getChild(child_index);
        Optional<Pair<DataBox, Long>> returned_keyrid = next_node.put(key, rid);
        // Case 1. Leaf node below did split and one key is returned
        if (!returned_keyrid.equals(Optional.empty())) {
            DataBox ret_key = returned_keyrid.get().getFirst();
            int key_index = numLessThanEqual(ret_key, this.getKeys());
            this.getKeys().add(key_index, ret_key);
            this.children.add(key_index + 1, returned_keyrid.get().getSecond());
            // Case 1-A: One key added / Inner node overflows
            if (getKeys().size() > 2*d) {
                DataBox mid_key = getKeys().get(d);
                getKeys().remove(d);
                InnerNode R = new InnerNode(this.metadata, this.bufferManager,
                        this.getKeys().subList(d, this.getKeys().size()), this.getChildren().subList(d + 1,
                        this.getChildren().size()), this.treeContext);
                this.keys = this.getKeys().subList(0, d);
                this.children = this.getChildren().subList(0, d + 1);
                Pair ret_pair = new Pair(mid_key, R.getPage().getPageNum());
                sync();
                return Optional.of(ret_pair);
            }

            // Case 1-B: One key added / Inner node didn't overflow
            sync();
            return Optional.empty();
        }
        // Case 2: No key is added so return
        return Optional.empty();
    }
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
            float fillFactor) {
        // TODO(proj2): implement
        Optional<Pair<DataBox, Long>> returned_keyrid = this.getChild(this.children.size() - 1).bulkLoad(data, fillFactor);
        // Case 1. Leaf node below did split and one key is returned
        if (!returned_keyrid.equals(Optional.empty())) {
            int d = this.metadata.getOrder();
            DataBox ret_key = returned_keyrid.get().getFirst();
            // Add the returned pair at the right end
            int key_index = this.children.size();
            this.getKeys().add(key_index - 1, ret_key);
            this.children.add(key_index, returned_keyrid.get().getSecond());
            // Case 1-A: One key added / Inner node overflows
            if (getKeys().size() > 2*d) {
                DataBox mid_key = getKeys().get(d);
                getKeys().remove(d);
                InnerNode R = new InnerNode(this.metadata, this.bufferManager,
                        this.getKeys().subList(d, this.getKeys().size()), this.getChildren().subList(d + 1,
                        this.getChildren().size()), this.treeContext); // End of pointer fixing code
                this.keys = this.getKeys().subList(0, d);
                this.children = this.getChildren().subList(0, d + 1);
                Pair ret_pair = new Pair(mid_key, R.getPage().getPageNum());
                sync();
