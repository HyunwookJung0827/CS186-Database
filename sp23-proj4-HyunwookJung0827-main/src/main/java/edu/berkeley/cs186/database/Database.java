    ...
    private class TransactionContextImpl extends TransactionContext {
        ...
        public void close() {
            try {
                ...
                List<Lock> allLocksFromThisTransaction = lockman.getLocks(this);

                for (int i = allLocksFromThisTransaction.size() - 1; 0 <= i; i--) {
                    LockContext.fromResourceName(lockman, allLocksFromThisTransaction.get(i).name).release(this);
                }
                //allLocksFromThisTransaction.iterator().next().name;

//                LockContext.fromResourceName(lockman, allLocksFromThisTransaction.iterator().next().name);
//                lockman.databaseContext();
//                this.getTransNum();
