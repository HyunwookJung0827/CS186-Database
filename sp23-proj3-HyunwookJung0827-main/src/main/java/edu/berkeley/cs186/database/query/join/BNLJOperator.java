...
public class BNLJOperator extends JoinOperator {
    ...
    private class BNLJIterator implements Iterator<Record>{
        ...
        private void fetchNextLeftBlock() {
            ...
            // Get (B-2) * (records per page) number of records and make them into a backtracking Iterator
            // then assign it to leftBlockIterator
            // getBlockIterator(Iterator<Record> records, Schema schema, int maxPages)

//            ArrayList<Record> leftRecords = new ArrayList<Record>();
//            int numRecords =  getLeftSource().estimateStats().getNumRecords();
//            int numPages = numBuffers - 2;
//            for (int i = 0; i < numRecords * numPages; i++) {
//                if (this.leftSourceIterator.hasNext()){
//                    leftRecords.add(this.leftSourceIterator.next());
//                } else return;
//            }
//            this.leftBlockIterator = getBlockIterator(leftRecords.iterator(), getLeftSource().getSchema(), numBuffers - 2);
//            this.leftRecord = this.leftBlockIterator.next();
        ...
        private void fetchNextRightPage() {
            // TODO(proj3_part1): implement
            // QueryOperator#getBlockIterator materialize(backtrackingIterator, etc.)
            /*if (this.rightSourceIterator == null || !this.rightSourceIterator.hasNext()) {
                return;
            }*/
            // Case A: In Case 3, next right page is on the rightside
            // Case B: In Case 4, next right page is in the beginning of the rightSourceIterator
            // In short, rightSourceIterator only resets in Case B.
            // else, do nothing.
            // Proceed to the same process in the fetchNextLeftBlock()

//            ArrayList<Record> rightRecords = new ArrayList<Record>();
//            int numRecords =  getLeftSource().estimateStats().getNumRecords();
//            for (int i = 0; i < numRecords; i++) {
//                if (this.rightSourceIterator.hasNext()){
//                    rightRecords.add(this.rightSourceIterator.next());
//                } else return;
//            }
//            this.rightPageIterator = getBlockIterator(rightRecords.iterator(), getRightSource().getSchema(), 1);
