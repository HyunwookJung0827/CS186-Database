...
public class SortOperator extends QueryOperator {
    ...
    public Run sortRun(Iterator<Record> records) {
        ...
        ArrayList<Record> fromMemory = new ArrayList<>();
        Run sortedRun = new Run(transaction, this.getSchema());
        fromMemory.add(records.next());
        Collections.sort(fromMemory, new RecordComparator());
        while (records.hasNext()) {
            fromMemory.add(records.next());
            Collections.sort(fromMemory, new RecordComparator());
        }
        sortedRun.addAll(fromMemory);
        return sortedRun;
    }
    ...
    public Run mergeSortedRuns(List<Run> runs) {
        ...
        Run mergedRun = new Run(transaction, this.getSchema());
        ArrayList<BacktrackingIterator<Record>> mergeList = new ArrayList<>();
        PriorityQueue<Pair<Record, Integer>> pq = new PriorityQueue<>(new RecordPairComparator());
        for (Run run : runs) {
            mergeList.add(run.iterator());
        }
        for (int i = 0; i < mergeList.size(); i++) {
            if (mergeList.get(i).hasNext()) {
                Pair<Record, Integer> validPair = new Pair<>(mergeList.get(i).next(), i);
                pq.add(validPair);
            }
        }
        Pair<Record, Integer> priorityP = pq.poll();
        Record priorityR = priorityP.getFirst();
        mergedRun.add(priorityR);
        int j = priorityP.getSecond();
        if (mergeList.get(j).hasNext()) {
            pq.add(new Pair<>(mergeList.get(j).next(), j));
        }
        while (!pq.isEmpty()) {
            priorityP = pq.poll();
            priorityR = priorityP.getFirst();
            mergedRun.add(priorityR);
            j = priorityP.getSecond();
            if (mergeList.get(j).hasNext()) {
                pq.add(new Pair<>(mergeList.get(j).next(), j));
            }
        }
    ...
    public List<Run> mergePass(List<Run> runs) {
        ...
        ArrayList<Run> sortedAndMerged = new ArrayList<>();
        int runSize = runs.size();
        for (int i = 0; i + numBuffers - 1 <= runSize + 1; i += numBuffers - 1) {
            List<Run> validList = runs.subList(i, Math.min((i + numBuffers - 1), runSize));
            sortedAndMerged.add(mergeSortedRuns(validList));
        }
