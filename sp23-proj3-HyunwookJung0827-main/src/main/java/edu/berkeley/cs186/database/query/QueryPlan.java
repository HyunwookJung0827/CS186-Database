...
public class QueryPlan {
    ...
    public QueryOperator minCostSingleAccess(String table) {
        ...
        for (int i = 0; i < indexColumns.size(); i++) {
            int curIndex = indexColumns.get(i);
            SelectPredicate curPredicate = this.selectPredicates.get(curIndex);
            QueryOperator select = new IndexScanOperator(this.transaction, table, curPredicate.column, curPredicate.operator, curPredicate.value);
            int estimatedIOCost = select.estimateIOCost();
            if (minCost > estimatedIOCost) {
                minOp = select;
                redundantIndex = curIndex;
                minCost = estimatedIOCost;
            }
        }
        minOp = addEligibleSelections(minOp, redundantIndex);
        return minOp;
    }
        ...
        */
        ...
        for (Set<String> tableSet : prevMap.keySet()) {
            for (int j = 0; j < this.joinPredicates.size(); j++) {
                JoinPredicate curPredicate = this.joinPredicates.get(j);
                QueryOperator curOperator; // = null;
                QueryOperator joined; // = null;
                Set<String> keysWithTables = new HashSet<>(tableSet);
                if (tableSet.contains(curPredicate.leftTable) && !tableSet.contains(curPredicate.rightTable)) {
                    curOperator = pass1Map.get(Collections.singleton(curPredicate.rightTable)); //.source;
                    joined = minCostJoinType(prevMap.get(tableSet), curOperator, curPredicate.leftColumn, curPredicate.rightColumn);
                    keysWithTables.add(curPredicate.rightTable);
                } else if (!tableSet.contains(curPredicate.leftTable) && tableSet.contains(curPredicate.rightTable)) {
                    curOperator = pass1Map.get(Collections.singleton(curPredicate.leftTable)); //.source;
                    joined = minCostJoinType(prevMap.get(tableSet), curOperator, curPredicate.rightColumn, curPredicate.leftColumn);
                    keysWithTables.add(curPredicate.leftTable);
                } else {
                    continue;
                }
                if (result.containsKey(keysWithTables)) {
                    if (result.get(keysWithTables).estimateIOCost() > joined.estimateIOCost()) {
                        result.put(keysWithTables, joined);
                    }
                } else {
                    result.put(keysWithTables, joined);
                }
    ...
    public Iterator<Record> execute() {
        ...
        Map<Set<String>, QueryOperator> lowestCostJoins = new HashMap<>();
        HashMap<Set<String>, QueryOperator> pass1Map = new HashMap<>();

        // Pass 1
        for (int i = 0; i < this.tableNames.size(); i++) {
            String currTable = this.tableNames.get(i);
            lowestCostJoins.put(Collections.singleton(currTable), minCostSingleAccess(currTable));
            pass1Map.put(Collections.singleton(currTable), minCostSingleAccess(currTable));
        }

        // Pass i
        for (int i = 1; i < this.tableNames.size(); i++) {
            lowestCostJoins = minCostJoins(lowestCostJoins, pass1Map);
        }

        // Rest of the function
        this.finalOperator = minCostOperator(lowestCostJoins);
        this.addGroupBy();
        this.addProject();
        this.addSort();
        this.addLimit();
