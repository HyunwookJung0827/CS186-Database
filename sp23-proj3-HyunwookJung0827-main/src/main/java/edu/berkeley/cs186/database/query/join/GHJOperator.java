...
//        }
        ...
        for (Record buildRecord : buildRecords) {
            DataBox buildCol = buildRecord.getValue(buildColumnIndex);
            if (!hashTable.containsKey(buildCol)) {
                hashTable.put(buildCol, new ArrayList<>());
            }
            hashTable.get(buildCol).add(buildRecord);
        }
        for (Record probeRecord : probeRecords) {
            DataBox probeCol = probeRecord.getValue(probeColumnIndex);
            if (!hashTable.containsKey(probeCol)) {
                continue;
            }
            for (Record buildRecord : hashTable.get(probeCol)) {
                Record matching;
                if (probeFirst) {
                    matching = probeRecord.concat(buildRecord);
                } else {
                    matching = buildRecord.concat(probeRecord);
                }
                this.joinedRecords.add(matching);
            }
        }
    }
    ...
    public static Pair<List<Record>, List<Record>> getBreakSHJInputs() {
        ...
        for (int i = 0; i < 201; i++) { // how many records? 201
    ...
    public static Pair<List<Record>, List<Record>> getBreakGHJInputs() {
        ...
        for (int i = 0; i < 43; i++) { // how many records? 43
